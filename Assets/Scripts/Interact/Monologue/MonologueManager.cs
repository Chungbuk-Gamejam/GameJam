...
public class MonologueManager : MonoBehaviour
{
    ...
    private Vector3 offset = new(0, 2.3f, 0); // 말풍선의 위치 오프셋
    ...
    private IEnumerator ActiveMonologue(string monologue)
    {
        ...
        Debug.Log("말풍선 위치 : " + $"{_playerTransform.position + offset}");
        foreach (char letter in monologue)
        {
            if (Input.GetKey(KeyCode.Space))
            {
                _playerController.monologuePanel.GetComponentInChildren<TMP_Text>().text = monologue;
                break;
            }
